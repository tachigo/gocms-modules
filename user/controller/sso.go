...
type SSOTestReq struct {
	g.Meta `path:"/sso-test" method:"GET" tags:"SSO测试" summary:"SSO用户信息回显" dc:"返回当前请求上下文中的SSO用户信息，用于验证Slave模式配置"`
}
...
type SSOTestRes struct {
	...
	Roles      []string          `json:"roles" dc:"解析后的角色列表"`
}
...
func (c *SSOTestController) SSOTest(ctx context.Context, req *SSOTestReq) (*SSOTestRes, error) {
	...
	rawHeaders := map[string]string{
		...
		"X-SSO-User-Roles":  r.GetHeader("X-SSO-User-Roles"),
		"Authorization":     r.GetHeader("Authorization"),
	}

	// 解析角色列表
	var roles []string
	if rolesStr := r.GetHeader("X-SSO-User-Roles"); rolesStr != "" {
